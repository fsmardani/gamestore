...
def phone_register(request):
    if request.method=='POST':
        ...
        print(code)
        print(phone_cache.get(f"user:{request.user.id}"))
        if code:
            if phone_cache.get(f"user:{request.user.id}") == code:
                supplier = SupplierProfile.objects.create(user=request.user.id,rate=3)
                supplier.save()
